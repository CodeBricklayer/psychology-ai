        ...
        <el-card v-if="aiData.systemOverview">
          <div class="card-content">
            <div class="avatar users">
              <el-image :src="userIcon" style="width: 40px; height: 40x;" />
            </div>
            <div class="info">
        ...
        <el-card v-if="aiData.systemOverview">
          <div class="card-content">
            <div class="avatar like">
              <el-image :src="likeIcon" style="width: 40px; height: 40x;" />
            </div>
            <div class="info">
        ...
        <el-card v-if="aiData.systemOverview">
          <div class="card-content">
            <div class="avatar comments">
              <el-image :src="commentsIcon" style="width: 40px; height: 40x;" />
            </div>
            <div class="info">
        ...
        <el-card v-if="aiData.systemOverview">
          <div class="card-content">
            <div class="avatar smile">
              <el-image :src="smileIcon" style="width: 40px; height: 40x;" />
            </div>
            <div class="info">
        ...
        <el-card style="width: 100%;">
          ...
          <div class="chart-content">
            <div class="consultation-stats">
              <div v-if="aiData.consultationStats" class="stat-item">
              ...
              <div v-if="aiData.systemOverview" class="stat-item">
                <div class="stat-label">活跃用户</div>
                <div class="stat-value">{{ aiData.systemOverview.activeUsers }}</div>
              </div>
            </div>
            <div ref="consultationChartRef" style="width: 100%; height: 260px;">
...
const commentsIcon = new URL('@/assets/images/comments.png', import.meta.url).href
...
const initEmotionChart = () => {
  ...
  // 获取情绪趋势的数据
  const ThendData = aiData.value.emotionTrend
  ...
  const option = {
    ...
    xAxis: {
      type: 'category',
      data: ThendData.map(item => item.date),
      axisLine: {
        lineStyle: {
          color: '#2d3436'
        }
    ...
    series: [
      {
        name: "平均情绪评分",
        type: 'line',
        data: ThendData.map(item => item.avgMoodScore),
        smooth: true,
        lineStyle: {
          width: 3,
          color: '#faebaf'
      ...
      },
      {
        name: "记录数量",
        type: 'line',
        data: ThendData.map(item => item.recordCount),
        smooth: true,
        lineStyle: {
          width: 3,
          color: '#eeb5a3'
...
const initUserActivityChart = () => {
  ...
  // 获取情绪趋势的数据
  const userTrend = aiData.value.userActivity
  ...
  const option = {
    ...
    series: [
      ...
      },
      {
        name: '日记用户',
        type: 'line',
        data: userTrend.map(item => item.diaryUsers),
        smooth: true,
        lineStyle: {
          width: 3,
          color: '#00b894'
      ...
      },
      {
        name: '咨询用户',
        type: 'line',
        data: userTrend.map(item => item.consultationUsers),
        smooth: true,
        lineStyle: {
          width: 3,
          color: '#fab1a0'
...
onMounted(() => {
  getDataAnalyticsOverview().then(res => {
    aiData.value = res
    initCharts()
  })
})
